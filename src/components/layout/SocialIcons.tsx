...
/**
 * Social platform glyphs, drawn inline.
 *
 * lucide-react removed its brand icons in v1 (trademark reasons), so these are
 * defined here rather than imported. Each is a single path on a 24×24 grid
 * using `currentColor`, so they inherit colour and size exactly like the lucide
 * icons used everywhere else and stay consistent in both themes.
 *
 * All are marked `aria-hidden` — the accessible name comes from the `aria-label`
 * on the surrounding link, so the platform is announced once, not twice.
 */
...
const icons: Record<SocialNetwork, (props: IconProps) => React.ReactElement> = {
